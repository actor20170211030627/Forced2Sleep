...
public class LaunchSelfUtils {
    ...
    public static boolean gotoLaunchList(Context context) {
        ...
        if (/*getMobileType().equals("Xiaomi")*/RomUtils.isXiaomi()) {// 红米Note4测试通过

            componentName = new ComponentName("com.miui.securitycenter", "com.miui.permcenter" +
                    ".autostart.AutoStartManagementActivity");

        } else if (/*getMobileType().equals("Letv")*/RomUtils.isLeeco()) {// 乐视2测试通过

            intent.setAction("com.letv.android.permissionautoboot");

        } else if (/*getMobileType().equals("samsung")*/RomUtils.isSamsung()) {// 三星Note5测试通过

            componentName = new ComponentName("com.samsung.android.sm_cn", "com.samsung" +
                    ".android.sm.ui.ram.AutoRunActivity");

        } else if (/*getMobileType().equals("HUAWEI")*/RomUtils.isHuawei()) {// 华为测试通过
...
//            componentName = ComponentName.unflattenFromString("com.iqoo.secure/.appmanager.AppManagerActivity");
        ...
        } else if (/*getMobileType().equals("OPPO")*/RomUtils.isOppo()) {// OPPO R8205测试通过

            componentName = ComponentName.unflattenFromString("com.oppo.safe/.permission" +
                    ".startup.StartupAppListActivity");

        } else if (/*getMobileType().equals("ulong")*/RomUtils.is360()) {// 360手机 未测试
